...
$update = mysqli_query($conn,"SELECT * FROM tb_karyawan WHERE username='$username'");
$data = mysqli_fetch_assoc($update);
$cek = mysqli_query($conn,"SELECT * FROM tb_karyawan WHERE role='admin'");
$cekrole = mysqli_num_rows($cek);
...
if(isset($_POST['submit'])){
	...
	$result = mysqli_query($conn, "UPDATE tb_karyawan
		SET password='$password', nama_karyawan='$nama_karyawan', role='$role' WHERE username='$username'");
...
<div class="row">
  ...
  <div class="col-md-10">
	...
	<table>
		<tr>
			<th colspan="3"><h4><b>Update Data Karyawan</b></h4></th>
		</tr>
		<tr>
			<td>Username</td>
			<td>:</td>
			<td><input type="text" name="username" value="<?php echo $data['username']; ?>" readonly>
			</td>
		</tr>
		<tr>
			<td>Password</td>
		...
		<tr>
			...
			<td>:</td>
			<td>
				<select name="role" <?php if ($cekrole == 1 && $data['role'] == 'Admin') {echo 'disabled';} ?>>
